...
import { placeHolderImage } from '@/lib/definition';
...

    {
      ...
      selector: row => 
        <React.Fragment> 
          ...
          {row.seller === 'Koro Gi' ? <button className='bg-green-700 text-base text-white py-2 px-4 rounded mr-1' onClick={() =>router.push(`/edit/${row.id}`)}>Edit</button> : null}
          {row.seller === 'Koro Gi' ? <button className='bg-red-700 text-base text-white py-2 px-4 rounded mr-1' onClick={() => handleDelete(row)}>Delete</button> : null}
        </React.Fragment>
    } 
